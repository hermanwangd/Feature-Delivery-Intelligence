# FDI MVP Hypothesis Freeze v0.4.7.1

**Status:** Governing experiment freeze before F001.
**Supersedes for future MVP execution:** `MVP-HYPOTHESIS-FREEZE-v0.4.6.2.md`
**Reason for revision:** v0.4.7.1 hardens Structural Intelligence source binding and removes an ablation confound before F001: repository identity is treated as shared neutral infrastructure rather than hidden Product Intelligence in the Structural-only arm.
...
### Shared Repository Identity Substrate

All four arms SHALL share the same **identity-only repository substrate** when canonical CandidateRepoSet normalization requires it. The reference substrate is the PA-03 CB-01 repository inventory projection at the same pre-feature cutoff. It exists only to normalize exact repository identity and legal `LAYER2_PA03` basis.

The shared substrate **must not include capability terms, Product Realization relations, Delivery Intelligence, semantic descriptions, candidate rankings, ownership inferences, or other Product Intelligence that could help discover scope**. Those remain treatment variables.

This prevents the Structural-only arm from secretly receiving semantic Product Intelligence merely because structural hints require PA-03 grounding.

F001 calibration SHALL use four controlled arms:

```text
A Baseline                  Semantic Product Intelligence NO   Structural Intelligence NO
B Structural only           Semantic Product Intelligence NO   Structural Intelligence YES
...
**F001 MUST NOT execute until this v0.4.7.1 H01–H05 freeze is active in the execution package.**
