...
Each row uses schemas `F` or `B` from section 4. Selected reads use section 3.1 patterns and the source selector in 3.2. Every literal-read path is enumerated as its own semicolon-delimited entry; when a row reads more than one revision, the revision qualifier is repeated on each entry. Group labels, member counts, directory-only tokens, and inferable bundle membership are not literal-read contracts. Revision-qualified dynamic sets belong in bounded selected reads, where every allowed path template, derivation source, exclusion, maximum, and concrete-match destination is explicit. A selector match is driven only by IDs/paths in a completed preceding artifact, a current literal-read registry, or a passed selector-preflight block written before dereference. Every row with nonempty bounded selected reads MUST literally read `.fdi/skills/context-selection/SKILL.md`. Registry-first selection is mandatory for every conditional leaf using the exact registry mapping in section 3.1, including the dedicated Knowledge, external-review, and Baseline-capability registries. Each gate's selection proof records registry path/revision, entry ID, `ACTIVE` lifecycle, applicability intersection, empty `superseded_by`, freshness/expiry and trust state, selected leaf/digest, and selection reason. Every concrete read is written to the output's `Context consulted` or `Source scope`. Planned edges are read only from `.fdi/features/{feature-id}/spec/design.md#planned-relations` and cannot satisfy a current-topology query. Every cross-file fragment is the explicit stable lowercase ID required by section 3.1.
...
| Every matrix literal-read path is individually enumerated, every non-current revision qualifier is repeated per path, and no group/count/directory shorthand or inferable bundle membership remains | PASS | Section 11 opening rule and all eight literal-read cells |
